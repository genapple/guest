...
def add_event(request):
    ...
    result=Event.object.filter(id=eid)
    if result:
    ...
    if status=='':
        ...
        try:
            ...
            error='start_time format error. It must be in  YYYY-MM-SS fromat. '
            return JsonResponse({'status':10024,'message':error})
        return JsonResponse({'status':'200','message':'add evenr success'})
#查询发布会接口
def get_event_list(requests):
...
def add_guest(request):
    ...
    if not result:
        return JsonResponse({'status':10023,'message':'event status is not available'})#可获得的，可购得的，可找到的
...
def get_guest_list(request):
    ...
    if eid !='' and  phone !='':
        ...
        try:
            ...
            return JsonResponse({'status':10022,'message':'qurey result is empty'})
        else:
...
def user_sign(request):
    ...
    result=Guest.objects.fillter(phone=phone)
    if not result:
        return JsonResponse({'status':10025,'message':'user phone null'})
    result=Guest.objects.filter(event_id=eid,phone=phone)
    if not result:
        return JsonResponse({'status':10026,'message':'user did not participate in the conference'})
    result=Geust.objects.get(event_id=eid,phone=phone).sign
    if result :
